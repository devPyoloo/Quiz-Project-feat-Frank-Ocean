...
export default function StartQuiz() {
  ...
  const [showFeedbackMessage, setShowFeedbackMessage] = useState<string | null>(null);
  ...
  if (!context) {
    // If context is undefined, throw an error or handle it gracefully
    throw new Error("QuizContext must be used within a QuizProvider");
  }

  const { quizState, setScore } = context;
  ...
  const handleSelectedChoice = (choiceIndex: number) => {
    const correctIndex =
